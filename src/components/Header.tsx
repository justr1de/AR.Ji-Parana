  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="bg-primary text-primary-foreground text-sm">
        <div className="container flex items-center justify-between py-2">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <span className="hidden md:inline">PREFEITURA DE JI-PARANÁ</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border-b border-border shadow-sm">
                ...
                <div key={item.label} className="relative group">
                  {item.submenu ? (
                    <>
                      ...
                      <div
                        className={`absolute top-full left-0 bg-card border border-border rounded-md shadow-lg py-2 min-w-[200px] transition-all ${
        ...
        {/* Mobile navigation */}
        {mobileMenuOpen && (
          <nav className="lg:hidden border-t border-border" aria-label="Menu mobile">
            <div className="container py-4 space-y-2">
              {menuItems.map((item) => (
                <div key={item.label}>
                  {item.submenu ? (
                    <details className="group">
                      <summary className="flex items-center justify-between px-3 py-2 text-sm font-medium text-foreground cursor-pointer list-none">
                        {item.label}
              ...
              <div className="pt-4 border-t border-border">
                ...
                >
                  ...
                  />
                  <span className="text-sm text-muted-foreground">Membro da ABAR</span>
                </a>
              </div>
            </div>
          </nav>
